...
import { DialogNodesConvert, TextModel } from 'src/app/Models/build-model/dialog-convert-model';
import { DialogNodes, OptionModel, OptionModelEnAR, ResOption, TemplateConvert, ValueEnAR } from 'src/app/Models/build-model/static-response-dialogs-modesl';
...
@Component({
  selector: 'vex-task-static-response',
  templateUrl: './task-static-response.component.html',
  styleUrls: ['./task-static-response.component.scss']
})
export class TaskStaticResponseComponent implements OnInit {
  ...
  dialogNodes:DialogNodes =new DialogNodes()
  tasks:AllTask[]
...
debugger
  ...
  getDialogs(){
    this._tasksService.getAllDialogs(this.workspace_id,this.intentId, 'Ex').subscribe((res:any)=>{
      debugger
      this.dialogNodes = res.dialog_nodes[0]
      if(this.dialogNodes){
        this.actionButtonMode = 'edit'
      }
      else{
        this.actionButtonMode = 'create'
      }
      this.convertData()
      this.getAllTasks()
      this.initializeForm()
  ...
  getAllTasks(){
    this._tasksService.getAllTasks(this.workspace_id,this.intentId).subscribe((res:any)=>{
      debugger
  ...
  submitForm(){
    ...
    if(this.actionButtonMode =='edit'){
      this._tasksService.editDialogs(this.workspace_id,this.intentId,'Ex',formObject,null,null).subscribe((res:any)=>{
        if(res.status == 1){
    ...
    if(this.actionButtonMode =='create'){
      var body = {
        intentMode:'Ex',
        ...
        dialog:{
          ...
          output:formObject.output
        }
      }
      this._tasksService.createDialog(body).subscribe((res:any)=>{
        if(res.status == 1){
          this.notify.openSuccessSnackBar('Data Saved')
          this.getDialogs()
        }
